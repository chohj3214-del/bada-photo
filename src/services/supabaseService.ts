...
type RemotePost = { id: string; image_url: string; created_at: string; location: string | null; author_name: string; custom_pose_allowed: boolean; caption?: string; hashtags?: string[]; is_public?: boolean; likes_count?: number; comments_count?: number };
...
export async function getPublicPosts(): Promise<UploadedPhoto[]> {
  ...
  return rows.filter((row) => row.is_public !== false).map((row) => ({ id: `remote-${row.id}`, dataUrl: row.image_url, createdAt: new Date(row.created_at).getTime(), location: row.location || undefined, customPoseAllowed: row.custom_pose_allowed, authorName: row.author_name, caption: row.caption, hashtags: row.hashtags, likesCount: row.likes_count, commentsCount: row.comments_count }));
}
